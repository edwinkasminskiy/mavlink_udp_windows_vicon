 ...
 *
 * @brief      helper functions for sending and unpacking the most common
 ...
 *
 *             This is a shortcut for rc_mav_send_heartbeat for those which
 *             don't wish to populate all of the availbale fields in the mavlink
 *             heartbeat packet. It still sends a complete heartbeat packet but
 *             with all 0's in the packet payload fields. The receiver can still
 *             see the system_id of the sender and this still serves the primary
 *             purpose to the heartbeat packet which is to indicate that a
 *             communication channel is open and functioning.
 *
 ...
 *             MAVLINK_MSG_ID_HEARTBEAT
 *
 *             Constructs and sends a heartbeat packet to the previously set
 *             destination ip address. The arguments encompass all available
 *             parameters in the heartbeat packet. However, many users will not
 *             be bothered to populate these parameters and may opt to use
 *             rc_mav_send_heartbeat_abbreviated() instead.
 *
 ...
 *                               indicates that none of the setpoint dimensions
 *                               should be ignored. If bit 10 is set the floats
 *                               afx afy afz should be interpreted as force
 *                               instead of acceleration. Mapping: bit 1: x, bit
 ...
 *                               if absolute or relative, above terrain if
 ...
 *                                 (positive for up). Note that virtually all
 *                                 GPS modules provide the AMSL altitude in
 ...
 *                                 direction of movement) in degrees * 100,
 *                                 0.0..359.99 degrees. If unknown, set to:
 *                                 UINT16_MAX
 ...
 *
 * @param[out] data  Pointer to user's packet struct to be populated with new data
 *
 * @return     0 on success, -1 on failure
 */
 ...
 *                                              Value of 0: not enabled. Value
 *                                              of 1: enabled. Indices defined
 *                                              by ENUM MAV_SYS_STATUS_SENSOR
 * @param[in]  load                             Maximum usage in percent of the
 *                                              mainloop time, (0%: 0, 100%:
 *                                              1000) should be always below
 *                                              1000
 * @param[in]  voltage_battery                  Battery voltage, in millivolts
 *                                              (1 = 1 millivolt)
 * @param[in]  current_battery                  Battery current, in
 *                                              10*milliamperes (1 = 10
 *                                              milliampere), -1: autopilot does
 *                                              not measure the current
 * @param[in]  drop_rate_comm                   Communication drops in percent,
 *                                              (0%: 0, 100%: 10'000), (UART,
 *                                              I2C, SPI, CAN), dropped packets
 *                                              on all links (packets that were
 *                                              corrupted on reception on the
 *                                              MAV)
 * @param[in]  errors_comm                      Communication errors (UART, I2C,
 *                                              SPI, CAN), dropped packets on
 *                                              all links (packets that were
 *                                              corrupted on reception on the
 *                                              MAV)
 ...
 *                      corresponds to forward(1000)-backward(-1000) movement on
 *                      a joystick and the pitch of a vehicle.
 ...
 *                      INT16_MAX indicates that this axis is invalid. Generally
 *                      corresponds to a separate slider movement with maximum
 *                      being 1000 and minimum being -1000 on a joystick and the
 *                      thrust of a vehicle.
 * @param[in]  r        R-axis, normalized to the range [-1000,1000]. A value of
 *                      INT16_MAX indicates that this axis is invalid. Generally
 *                      corresponds to a twisting of the joystick, with
 *                      counter-clockwise being 1000 and clockwise being -1000,
 *                      and the yaw of a vehicle.
 * @param[in]  buttons  A bitfield corresponding to the joystick buttons'
 *                      current state, 1 for pressed, 0 for released. The lowest
